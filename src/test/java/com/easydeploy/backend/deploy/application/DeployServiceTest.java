...
class DeployServiceTest {
    ...
    void setUp() {
        deployService = new DeployService(
                ...
                serverService, new SyncTaskExecutor());
    }
    ...
    @Test
    void 상태_조회_존재하지_않는_jobId_예외() {
        when(deployJobRepository.findById(999L)).thenReturn(Optional.empty());

        assertThatThrownBy(() -> deployService.getStatus(999L))
    ...
    void 배포_취소_CANCELLED_상태로_변경() {
        ...
        when(deployJobRepository.findById(1L)).thenReturn(Optional.of(job));
        when(deployJobRepository.save(any())).thenReturn(job);
        when(terraformService.getJobDir(any())).thenReturn(java.nio.file.Path.of("/tmp/nonexistent"));

        DeployResponse resp = deployService.cancelDeploy(1L, "user@test.com");
    ...
    void 다른_사용자_배포_취소_실패() {
        ...
        assertThatThrownBy(() -> deployService.cancelDeploy(1L, "other@test.com"))
                .isInstanceOf(IllegalArgumentException.class);
    }
